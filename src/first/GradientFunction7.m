function [f g] = GradientFunction7(x, kx, kf, M, frequency_offset, x_positions)
% Calculates objective function as
%
% abs( M - sum_j sum_p m_j  exp(-i2pi( x*kx + (delta_f + phi_x)*kf) - R2*TE)
% 
% Species, field and R2* are estimated jointly. Note that R2* is multiplied
% by TE, not kf. Please refer to GradientFunction7v2 for corrections.
...
if nargout > 1 % gradient required
    ...
    for ii = 1:Nacq
        ...
        for jj = 1:Mspecies          
            ...
            % phi
            deriv_phi = (-midospi*(kf(ii,:).'*m(jj,:)).').*deriv_x;
