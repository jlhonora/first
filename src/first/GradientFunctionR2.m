function [f g] = GradientFunctionR2(x, kx, kf, M, frequency_offset, x_positions, phi_x)
% Calculates objective function as
%
% abs( M - sum_j sum_p m_j  exp(-i2pi( x*kx + (delta_f + phi_x)*kf) - R2*TE)
% 
% phi_x is assumed as correct.
%
% Note that R2 is multipled by TE, not kf. Please refer to
% GradientFunctionR2v2 for corrections.
...
%             g((1:2*N)+2*N*(jj-1)) = g((1:2*N)+2*N*(jj-1)) - [
%                 (diff_m.'*deriv_x'+diff_m'*deriv_x.').';
%                 (diff_m.'*deriv_x'+diff_m'*deriv_x.').'];
            % R2
%             TEm = repmat(-TE(ii)*m(jj,:).',1,N);
%             g((1:N)+N+2*N*(Mspecies)) = g((1:N)+N+2*N*(Mspecies)) - ...
%                     (conj(TEm.*deriv_x)*(diff_m) + (TEm.*deriv_x)*conj(diff_m)); 
%             TEm = -TE(ii)*m(jj,:).';
%             g((1:N)+N+2*N*(Mspecies)) = g((1:N)+N+2*N*(Mspecies)) - ...
%                     (conj(deriv_x)*(diff_m.*conj(TEm))...
%                    + deriv_x*(conj(diff_m).*TEm) );
%             TEm = (-TE(ii)*m(jj,:))*deriv_x;
%             g((1:N)+N+2*N*(Mspecies)) = g((1:N)+N+2*N*(Mspecies)) - ...
%                     (conj(TEm.').*(diff_m)...
%                    + (TEm.').*conj(diff_m) );   
            TEm = (-TE(ii)*m(jj,:)).';
            g((1:N)+N+2*N*(Mspecies)) = g((1:N)+N+2*N*(Mspecies)) - ...
                    ((conj(deriv_x)*diff_m).*conj(TEm)+(deriv_x*conj(diff_m)).*(TEm));   
...
%     g((1:N)+N+2*N*(Mspecies)) = fftshift(ifft(fftshift(g((1:N)+N+2*N*(Mspecies)))));
%     g((1:N)+N+2*N*(Mspecies)) = abs(g((1:N)+N+2*N*(Mspecies)));
    %g = 2*g;
