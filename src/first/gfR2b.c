/*=========================================
 * gfR2c.c : C-version of GradientFunctionR2v2.m (Jose Luis)
 *
 ...
 * phi : local field inhomogeneity (size 1xN)
 *
 * OUTPUT
 * f : cost function value (pointer)
 * ========================================== */
void gfR2b(double *x, double *kx, double *kf, double *Mre, double *Mim,
			...
			for (q=0; q<N; q++) { /* For every k position */
				...
				for (m=0; m<M; m++) { /* For every species */
					...
					for (n=0; n<N; n++) { /* For every x positions (real and imaginary) */
						...
						im_gR2 = im_gR2 - tmp_kf*(im_rho_m*cs + re_rho_m*sn);
						re_gR2 = re_gR2 - tmp_kf*(re_rho_m*cs - im_rho_m*sn);
...
/* function f = GradientFunctionR2v2(x, kx, kf, M, frequency_offset, x_positions, phi) */
	...
	//if( mxIsComplex(prhs[5]) ) { /* Warning for complez */
	//	mexWarnMsgTxt("gf5 WARNING: Imaginary part of xpos (input 6) ignored."); }
	phi = mxGetPr(prhs[6]); /* creates pointer */
	i = mxGetM(prhs[6]); /* get size */
	j = mxGetN(prhs[6]);
	...
	gfR2b(x, kx, kf, Mre, Mim, fo, xpos, phi, N, M, Nacq, f, g);
