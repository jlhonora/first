/*=========================================
 * gfR2.c : C-version of GradientFunctionR2v2.m (Jose Luis)
 *
 * Based on Pablo Irarrazaval's gf5.c
 *
 * 2011 06 13
 *
 * JLH
 ...
 * ========================================== */
void gfR2(double *x, double *kx, double *kf, double *Mre, double *Mim,
		 double *fo, double *xpos, double *phi, mwSize N, mwSize M, mwSize Nacq,
		 double *f, double *g)	 
{
	...
	double re_rho_m, im_rho_m, re_gR2, im_gR2, expR2; 
	double Dqr,Dqi;
...
// 	printf("C: N = %d M = %d Nacq = %d\n",N,M,Nacq);
	...
	if( g==NULL ) { /* only cost function */
			...
			for (q=0; q<N; q++) { /* For every k position */
				...
				for (m=0; m<M; m++) { /* For every species */
					...
					for (n=0; n<N; n++) { /* For every x positions */
						...
						mr = x[twoNm+n];
						mi = x[twoNm+N+n];
						ang = -twopi*(xpos[n]*tmp_kx - tmp_fo_kf - (phi[n])*tmp_kf);
		...
		for (n=0; n<M*N*2+2*N; n++) { /* makes sure g starts in zero */
			g[n] = 0; }
		/* allocates memory for temporal arrays */
		pMEr = (double *)malloc(M*N*2*sizeof(double));
		pMEi = (double *)malloc(M*N*2*sizeof(double));
		...
		for (e=0; e<Nacq; e++) { /* For every echo */
			...
			for (q=0; q<N; q++) { /* For every k position */
				...
				for(i=0; i<M*N*2; i++){ /* initialize temporal array */
					pMEr[i] = 0;
					pMEi[i] = 0; }
				...
				for (m=0; m<M; m++) { /* For every species */
					...
					for (n=0; n<N; n++) { /* For every x positions (real and imaginary) */
						...
						expR2 = exp(-x[twoNM+N+n]*tmp_kf);
						im_gR2 = im_gR2 + tmp_kf*(im_rho_m*cs + re_rho_m*sn)*expR2;
						re_gR2 = re_gR2 + tmp_kf*(re_rho_m*cs - im_rho_m*sn)*expR2;
						g[twoNM+N+n] = g[twoNM+N+n] - Dqr*re_gR2 - Dqi*im_gR2; // R2
...
/* function f = GradientFunctionR2v2(x, kx, kf, M, frequency_offset, x_positions, phi_x) */

	
	/* variables to pass into the computational function */
	double *x,*kx,*kf,*Mre,*Mim,*fo,*xpos,*phi; /* directly from Matlab script */
	...
	gfR2(x, kx, kf, Mre, Mim, fo, xpos, phi, N, M, Nacq, f, g);
